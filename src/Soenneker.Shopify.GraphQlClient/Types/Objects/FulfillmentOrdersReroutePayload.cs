// <auto-generated />
#nullable enable
using System.Collections.Generic;
using System.Text.Json.Serialization;

namespace Soenneker.Shopify.GraphQlClient;

/// <summary>
/// Return type for `fulfillmentOrdersReroute` mutation.
/// </summary>
public sealed partial class FulfillmentOrdersReroutePayload
{
    /// <summary>
    /// The fulfillment orders which contains the moved line items.
    /// </summary>
    [JsonPropertyName("movedFulfillmentOrders")]
    public List<FulfillmentOrder>? MovedFulfillmentOrders { get; init; }

    /// <summary>
    /// The list of errors that occurred from executing the mutation.
    /// </summary>
    [JsonPropertyName("userErrors")]
    public List<FulfillmentOrdersRerouteUserError> UserErrors { get; init; } = [];

}
